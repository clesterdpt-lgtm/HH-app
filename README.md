...
- **Visit Session**: Start a visit session with label, visit type, and date; session labels auto-apply to all clinical sections (Notes, Meds, HEP, Education) and the chosen visit type auto-syncs the Notes module note type selector. A persistent context bar appears on clinical modules with Edit and End controls. Label input uses tag-style creation consistent with other label fields
...
- **Encryption lock screen**: Password required at app startup to decrypt clinical data; key held in memory only. "Continue without decryption" option hides all history records (not just encrypted ones) for read-only access to unencrypted features
...
docs/
  ...
  guardrails-recommendations.md     # Legal, clinical, and product guardrails
  positioning-recommendations.md    # Product positioning guidance
supabase/
