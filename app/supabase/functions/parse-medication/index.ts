...
serve(async (req) => {
  ...
  try {
    ...
    const { transcript, bulk } = await req.json();
    ...
    const systemPrompt = bulk
      ? `You are a clinical documentation assistant. The user has recorded audio of a full medication list being read aloud. Extract every medication mentioned and return ONLY a JSON array of objects. Each object must have exactly these keys: name, dose, frequency, route, notes. For route, use one of: Oral, Topical, Inhaled, Injection, Sublingual, Transdermal, Other. If a field cannot be determined for a given medication, use an empty string for that field. Return nothing except the raw JSON array — no markdown, no explanation, no wrapper object.`
    ...
    if (bulk) {
      let medications;
      try {
        medications = JSON.parse(rawText);
      } catch {
        return new Response(JSON.stringify({ error: "Failed to parse medications JSON from Claude response", raw: rawText }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
